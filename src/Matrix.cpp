...
matrix operator+(const matrix& matrix1, const matrix& matrix2) 
{
 ...
 { 
 	...
 	//先判断是否为方阵 
 	{ 
 		std::cout << "矩阵不是方阵不可进行求逆运算！" << std::endl; 
 		exit(1); 
 	} 
 	 	matrix M(Col);
 	for(int i=0;i<Col;i++)
 		for(int j=0;j<Col;j++)
 		{
 			M.SetElem(i+1,j+1,GetElem(i+1,j+1));
 		}
 	...
 	{ 
 		...
 		{ 
 			...
 			{ 
 				if(abs(Max) < abs(M.GetElem(l+1,i+1))) 
 				//找出每一列主元素所在列的向下最大元素 
 				{ 
 					Max = M.GetElem(l+1,i+1); 
 					Max_Label = l; 
 					for(int k = 1; k <= Col; k++) 
 					{ 
 						long double* Swap = new long double; 
 						*Swap = M.GetElem(Max_Label+1,k); 
 						M.SetElem(Max_Label+1,k,M.GetElem(i+1,k)); 
 						M.SetElem(i+1,k,*Swap); 
 			...
 			{ 
 				...
 				{ 
 					if(M.GetElem(l+1,i+1) != 0) 
 					{ 
 						long double a = M.GetElem(l+1,i+1); 
 						for(int k = 0; k < Col; k++) 
 						{ 
 							//long double temp = Max * GetElem(l+1,k+1)/a - GetElem(i+1,k+1); 
 							M.SetElem(l+1,k+1,Max * M.GetElem(l+1,k+1)/a - M.GetElem(i+1,k+1)); 
 		...
 		{ 
 			...
 			{ 
 				...
 				{ 
 					for(int k = 1; k <= Col; k++) 
 					{ 
 						M.SetElem(j,k,Max * M.GetElem(j,k)/a - M.GetElem(Col,k)); 
 	...
 	for(int i = 1; i <= Col; i++) 
 	{ 
 		long double a = M.GetElem(i,i); 
 		for(int j = 1; j <= Col; j++) 
 		{ 
 			M.SetElem(i,j,M.GetElem(i,j)/a); 
...
matrix matrix::Expand(int NewSize,double Value)
{
	if(GetRows()!=GetCols())
	{
		std::cout<<" 矩阵不是方阵不可kuozhan！"<<std::endl;
		return 0;
	}

	matrix E(NewSize,NewSize);
